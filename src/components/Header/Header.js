      ...
      <div id="top" className="header_navigate">
        ...
        <div className={`header_navigate_menu${menuOpen ? ' open' : ''}`}>
          <nav className="header_navigate_menu_link">
            <NavLink
              dir="auto"
              className={({ isActive }) => isActive ? 'menuLinkActive' : 'header_navigate_menu_link_noLink'}
              to="/" onClick={menuOpen ? toggleMenu : null}
            >
              {t('header-generate')}
            </NavLink>
            <NavLink 
              className={({isActive}) => isActive ? 'menuLinkActive' : 'header_navigate_menu_link_noLink'} 
              to="/privacy" onClick = {menuOpen? toggleMenu: null }
            >
              {t('header-privacy')}</NavLink>
            <NavLink 
              className={({isActive}) => isActive ? 'menuLinkActive' : 'header_navigate_menu_link_noLink'} 
              to="/pricing" onClick = {menuOpen? toggleMenu: null }
            >
              {t('header-pricing')}</NavLink>
            <NavLink 
              className={({isActive}) => isActive ? 'menuLinkActive' : 'header_navigate_menu_link_noLink'} 
              to="/blog" onClick = {menuOpen? toggleMenu: null }
            >
              {t('header-blog')}
            </NavLink>
          </nav>
          <div className="header_navigate_menu_switcher">
            <select
              className="header_navigate_menu_switcher_selected"
          ...
          <div className="header_navigate_menu_button">
            {user ? 
              <button
                className="header_navigate_menu_button_signup"
              ...
              <>
                <h2
                  className="header_navigate_menu_button_link"
                  onClick={openAccount}
                >
                  {t('account-open')}
                  <span>v</span>
                ...
                {account &&
                  <>
                    <Link 
                      to="/sign_in" 
                      className="header_navigate_menu_button_login" 
                    ...
                    <button
                      className="header_navigate_menu_button_signup"
                      onClick={() => { navigate('/sign_up'); if (menuOpen) toggleMenu(); }}
                    >
                      {t('header-signUp')}
                    </button>
                  </>
